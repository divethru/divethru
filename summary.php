...
<meta name="twitter:url" content="http://34.215.40.163/summary.php">
...
<script type="text/javascript">
	...
	var qmsg = Sessiondata.session_quote_description+' #'+Sessiondata.session_name+'@diveThru';
		$.redirect("twitter_login.php",{qimg:Sessiondata.session_quote_img ,msg:qmsg},"POST",null,null,true);
				...
				if((!window.localStorage.getItem("cat") || window.localStorage.getItem("cat") == "10 Day Intro Program") && (user.last_free_conversation_id == 4 || user.last_free_conversation_id == 8 || user.last_free_conversation_id == 10) && user.membership_type == "Free"){
				//alert(5);
					...
					if(window.localStorage.getItem("cat") == "Deep Dive"){
					  ...
					  $.redirect(referrer,{"bundle":bid},"POST",null,null,true);

						//history.go(-1);
					}else if(window.localStorage.getItem("cat") == "Quick Dive"){
						var referrer = window.localStorage.getItem("referrer");
						var bid = window.localStorage.getItem('session_id');
					  $.redirect(referrer,{"Qsid":bid},"POST",null,null,true);
						//history.go(-2);
					...
					//	alert(window.localStorage.getItem('colorcode'));
						history.go(-1);
...
<div class="container-fluid px-0">
    <div class="row mx-0" style="color: rgb(90, 97, 117);">
		...
		<div class="col-lg-6 part2 text-center">
			...
			<a href="javascript:void(0)" id="SModal" class=" btn1 btn-primary1 px-4 py-2 mx-2"><i class="fa fa-paper-plane mr-2" aria-hidden="true"></i>SHARE</a>
...
twitterShare.onclick = function(e) {
 ...
 // window.open("https://twitter.com/intent/tweet?via=ProgramCafe");
  var twitterWindow = window.open('https://twitter.com/intent/tweet?text='+Sessiondata.session_quote_description+';hashtags='+Sessiondata.session_name+'&url=' + document.URL+'&via=diveThru', 'twitter-popup', 'height=350,width=600');
				...
				//alert(5);
					...
					if(window.localStorage.getItem("cat") == "Deep Dive"){
					  ...
					  $.redirect(referrer,{"Qsid":bid},"POST",null,null,true);
						...
						//alert("AF"+window.localStorage.getItem('colorcode'));
						history.go(-1);
...
var BASE_URL = "http://34.215.40.163/summary.php";
var title = "DiveThru";
